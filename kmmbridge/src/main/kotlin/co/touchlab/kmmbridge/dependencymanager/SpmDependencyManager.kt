...
internal class SpmDependencyManager(
    ...
    override fun configure(
        ...
        registerInSpmRegistry(project, packageName, swiftToolVersion)
        ...
        val writeMetadataTask =
            project.tasks.register("writeSpmMetadata") {
                ...
                inputs.property("frameworkName", packageName)
                inputs.property("platforms", platformsMap)
                inputs.property("swiftToolsVersion", swiftToolVersion.name)
        ...
        val updatePackageSwiftTask =
            project.tasks.register("updatePackageSwift") {
                ...
                doLast(
                    object : Action<Task> {
                        override fun execute(t: Task) {
                            // Skip if the root SPM aggregator plugin is applied (use generatePackageSwift/
                            // kmmBridgePublishAll instead) to avoid racing/overwriting the aggregated file.
                            if (registry.get().isRootApplied()) {
                                t.logger.lifecycle(
                                    "Skipping updatePackageSwift: root project applies co.touchlab.kmmbridge.spm. " +
                                        "The aggregated Package.swift is generated by 'generatePackageSwift'/'kmmBridgePublishAll' instead.",
                                )
                                return
                            }
     ...
     */
    private fun registerInSpmRegistry(project: Project, packageName: String, swiftToolsVersion: SwiftToolVersion) {
        ...
        project.kmmBridgeSpmRegistry().get().registerModule(
            KmmBridgeSpmRegistry.ModuleRegistration(
                ...
                swiftToolsVersion = swiftToolsVersion.name,
            ),
        )
    }
    ...
    fun configureLocalDev(project: Project) {
        ...
        registerInSpmRegistry(project, project.kmmBridgeExtension.frameworkName.get(), swiftToolVersion)
