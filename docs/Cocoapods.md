...
```kotlin
kmmbridge {
    faktoryReadKey.set("0123456789ABCDEF")
...
To publish a new version, use the `publishRemoteFramework` <!-- TODO task will be renamed in issue #197 --> gradle task.
This will take care of uploading a new binary to the Faktory server, as well as uploading the podspec to the designated
...
the `source` and `version` will overridden by the Faktory plugin.

### Version numbering

The Faktory plugin does some automated version management in order to ensure that new library versions are published
